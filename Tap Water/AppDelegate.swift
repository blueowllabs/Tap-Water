...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        if (settings!.types.intersect(UIUserNotificationType.Alert) != [] && GetSettingsAttribute(Notification, key: NotificationKey)) {
    ...
    func UpdateAppBadge(application: UIApplication) {
        let day = GetCurrentDay()
        let glassesLeft = day.totalGlassesGoal - day.totalGlassesDrank
        let settings = UIApplication.sharedApplication().currentUserNotificationSettings()
        
        if (settings!.types.intersect(UIUserNotificationType.Badge) != []) {
            if glassesLeft > 0 {
    ...
    lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource("Tap_Water", withExtension: "momd")!
    ...
    lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator? = {
        ...
        let url = self.applicationDocumentsDirectory.URLByAppendingPathComponent("Tap_Water.sqlite")
